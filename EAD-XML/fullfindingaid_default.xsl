<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	two columns. -->
            <body>
                ...
                <!--This
                part of template inserts a logo and title
				at the top of the display.  Insert the proper path ro
				your image in place of yourlogo.gif.-->

                <!--If
                you do not want to include an image, delete the center
    ...
    <!--This
    template rule formats a list element anywhere
    ...
    <!--Formats
    a simple table. The width of each column is defined by the colwidth attribute in a colspec
    element.-->
    ...
    <!--This
    template rule formats a chronlist element.-->
    ...
    <xsl:template match="chronitem">
        <!--Determine
        if there are event groups.-->
        <xsl:choose>
            <xsl:when test="eventgrp">
                <!--Put
                the date and first event on the first line.-->
                <tr>
                    <td> </td>
                    <td valign="top">
                ...
                <!--Put
                each successive event on another line.-->
            ...
            <!--Put
            the date and event on a single line.-->
            <xsl:otherwise>
                <tr>
                    <td> </td>
                    <td valign="top">
    ...
    <!--Suppreses
    all other elements of eadheader.-->
    <!--
    ...
    <!--<xsl:template
    match="titlepage/list/item  | titlepage/list/head">-->
    ...
    <xsl:template match="eadheader/profiledesc/creation  | eadheader/profiledesc/langusage">
        <!--The
        template tests to see if there is a label attribute,
    ...
    <!--This
    template creates a table for the did, inserts the head and then
...
elements, change the sequence of the apply-templates statements.-->
    <xsl:template match="archdesc/did">
        <table width="100%">
            ...
            <!--One
            can change the order of appearance for the children of did
    ...
    <!--This
    template formats the repostory, origination, physdesc, abstract,
...
of unittitle and unitdate.-->
    <xsl:template match="archdesc/did/unittitle">
        <!--The
        template tests to see if there is a label attribute for unittitle,
inserting the contents if there is or adding one if there isn't. -->
        <xsl:choose>
            <xsl:when test="@label">
                <tr>
                    ...
                    <td>
                        <!--Inserts
                        the text of unittitle and any children other that unitdate.-->
        ...
        <!--If
        unitdate is a child of unittitle, it inserts unitdate on a new line.  -->
    ...
    <!-- Processes the unit date if it is not a child of unit title but a child of did, the current
    context.-->
    <xsl:template match="archdesc/did/unitdate">

        <!--The
        template tests to see if there is a label attribute for a unittitle that is the
    ...
    <!--This
    template processes the note element.-->
...
inserting the contents if there is or adding one if there isn't. -->
            <xsl:choose>
                <xsl:when test="parent::note[@label]">
                    <!--This
                    nested choose tests for and processes the first paragraph. Additional paragraphs
                    do not get a label.-->
                    <xsl:choose>
    ...
    <!--This
    template rule formats the top-level bioghist element and
    ...
    <!--This
    template formats various head elements and makes them targets for
    ...
    <!--This
    template rule formats the top-level related material
    ...
    <xsl:template name="archdesc-relatedmaterial">
        ...
        string(archdesc/separatedmaterial) or   string(archdesc/*/separatedmaterial)">-->
        <xsl:if test="string(archdesc/relatedmaterial) or   string(archdesc/*/relatedmaterial)">
            ...
            <!--<xsl:apply-templates
            select="archdesc/separatedmaterial/p     | archdesc/*/separatedmaterial/p     |
            archdesc/separatedmaterial/note/p     | archdesc/*/separatedmaterial/note/p"/>-->
            <hr />
        </xsl:if>
    </xsl:template>
    <xsl:template name="archdesc-separatedmaterial">
        ...
        <xsl:if test="string(archdesc/separatedmaterial) or   string(archdesc/*/separatedmaterial)">
            ...
            <!--<xsl:apply-templates
            select="archdesc/relatedmaterial/p     | archdesc/*/relatedmaterial/p     |
            archdesc/relatedmaterial/note/p     | archdesc/*/relatedmaterial/note/p"/>-->
            <xsl:apply-templates
    ...
    <!--This
    template formats the top-level controlaccess element.
    ...
    <xsl:template match="archdesc/controlaccess">
        <xsl:if test="string(child::*)">
            ...
            <xsl:choose>
                <!--Apply
                this template when there are recursive controlaccess
				elements.-->
                <xsl:when test="controlaccess">
                    <xsl:apply-templates mode="recursive" select="." />
                </xsl:when>
                <!--Apply
                this template when the controlled terms are entered
    ...
    <!--This
    template formats controlled terms that are entered
    ...
    <!--When
    controlled terms are nested within recursive
    ...
    <!--This
    template formats controlled terms that are nested within recursive
    ...
    <!--This
    template rule formats a top-level access and use retriction elements.
    ...
    <xsl:template name="archdesc-restrict">
        <xsl:if
            test="string(archdesc/userestrict/*)   or string(archdesc/accessrestrict/*)   or string(archdesc/*/userestrict/*)   or string(archdesc/*/accessrestrict/*)">
            <h3>
                <b>
    ...
    <!--This
    templates consolidates all the other administrative information
    ...
    <!--This
    template rule formats the head element of top-level elements of
    ...
    <!--This
    template rule tests for and formats the top-level index element. It begins
        ...
        <table width="100%">
            ...
            <xsl:for-each select="indexentry">
                ...
                <tr>
                    ...
                    <!--Supplies
                    whitespace and punctuation if there is a pointer
                    ...
                    <xsl:choose>
                        ...
                        <!--If
                        there is no pointer group, process each reference or pointer.-->
                        <xsl:otherwise>
                            <td>
                                <xsl:for-each select="ref | ptr">
                                    <xsl:apply-templates />
